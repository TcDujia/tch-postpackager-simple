...
//        }
        if (toCombine.length !== 0) {
            ...
            if (combineCache[hash]){
                ...
                var key;
                for(var i = 0,len = has.length -1;i<=len;i++){
                    var _tmp = /tc\/(\w+)/.exec(has[i]);
                    var item = has[i],
                        lastIndex = item.lastIndexOf("/"),
                        _key = item.substr(0,lastIndex);
                    if(_key.indexOf("tpl")>-1||_key.indexOf("common")>-1||_key.indexOf("global")>-1||_key.indexOf("utils")>-1||_key.indexOf("async")>-1){
                        continue;
                    }
                    key = _key.replace("modules/","").replace(/\//g,"_");
                    break;
                }
                var subpath;
                //debugger;
                if(key){
                    subpath = 'pkg/'+key+'.'+fileExt;
                }else{
                    subpath = 'pkg/auto_combine_${index}'.replace('${index}', combineCount) + '.' + fileExt;
                }
